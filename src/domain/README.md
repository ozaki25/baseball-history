...
- `query/` — 絞り込み・URL 変換・観戦/予定分割（`filter.ts`, `search.ts`, `options.ts`, `partition.ts`）
- `stats/` — 基本統計・軸別集計・表示行組み立て（`summary.ts`, `axes.ts`, `rows.ts`）

トップ直下のファイル: `game.ts`（列挙・型・述語）, `labels.ts`（表示語彙）, `masters.ts`（安定 ID）, `normalize.ts`（正規化）。
