...
```
┌───────────────────────────────────────────────┐
│ Header（タイトル / ファイターズカラー）          │
├───────────────────────────────────────────────┤
│ Filters: [年度・期間][球場▼][相手▼][H/V][勝敗▼] [リセット] │
├───────────────────────────────────────────────┤
│ StatsSummary: 観戦 N / 勝 W 敗 L 分 D / 勝率 .xxx │
├───────────────────────────────────────────────┤
│ CrossStats（タブ or 折りたたみ）:                 │
│   球場別 / 相手別 / 年度別 / H・V 別 の集計テーブル │
├───────────────────────────────────────────────┤
│ GameTable: 絞り込み後の試合一覧（並べ替え可）      │
│   PC=テーブル / モバイル=カード                    │
├───────────────────────────────────────────────┤
│ Footer                                          │
└───────────────────────────────────────────────┘
...
- Filters・Stats・Table はすべて**同じ絞り込み状態（URL）に連動**。
- アクセシビリティ: フィルタは適切なラベル/role、テーブルは `scope`・キャレット操作対応。
